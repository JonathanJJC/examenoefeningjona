...
$view_profile = $db->select("SELECT gebruiker.id, gebruiker.voornaam, gebruiker.achternaam, gebruiker.email, gebruiker.created_at, image.Filename, groep.naam, post.inhoud, groepsleden.lid_id
  FROM gebruiker
  LEFT JOIN image
  ON gebruiker.id = image.id 
  LEFT JOIN post
  ...
  LEFT JOIN groep
  ON groepsleden.groeps_id = groep.id
  WHERE gebruiker.id = :id", [':id' => $view_profile]);

$columns_view_profile = array_keys($view_profile[0]);  
$row_data_view_profile = array_values($view_profile);
      ...
      <div class="w3-card w3-round w3-white">
        <div class="w3-container">
         ...
         <?php  if ($data_view_profile["Filename"] == NULL) {
                ...
                echo "<img src=image/$data_view_profile[Filename] class=\"w3-circle\" style=\"max-height:106px;max-width:106px;\" alt=Avatar><br>";
              }?>	
         	<!-- <img src="image/<?php echo $data_view_profile["Filename"];?>" class="w3-circle" style="height:106px;width:106px" alt="Avatar"></p> -->
         <hr>
         <p><i class="fa fa-address-card fa-fw w3-margin-right w3-text-theme"></i><?php echo $data_view_profile["voornaam"], " ", $data_view_profile["achternaam"]?></p>
         <p><i class="fa fa-envelope fa-fw w3-margin-right w3-text-theme"></i><?php echo $data_view_profile["email"]?></p>
         <p><i class="fa fa-calendar fa-fw w3-margin-right w3-text-theme"></i>Lid sinds: <?php echo $data_view_profile["created_at"]?></p>
        </div>
      </div>
      <br>
      ...
      <div class="w3-row-padding">
       ...
       <?php
        foreach ($row_data_view_profile as $data_view_profile) {
          ...
          if ($data_view_profile["Filename"] == NULL) {
                echo "<img src=image/default.png class=\"w3-left w3-circle w3-margin-right\" style=width:60px;height:60px;>";
              }else{
                echo "<img src=image/$data_view_profile[Filename] class=\"w3-left w3-circle w3-margin-right\" style=width:60px;height:60px;><br>";
              }	
          echo "<span class=\"w3-right w3-opacity\">32 min</span>";
          echo "<h4>".$data_view_profile["voornaam"], " ", $data_view_profile["achternaam"]."</h4><br>";
